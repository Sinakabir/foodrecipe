  ...
  return (
    ...
    <img src={item?.image_url} alt="recipe item" className='block w-full' />
      ...
      <div>
        ...
        <h3 className='fotn-bold text-2xl truncate text-black'>{item?.title}</h3>
        <Link to={`/recipe-item/${item?.id}`} className='text-sm p-3 px-8 rounded-lg uppercase font-medium tracking-wider inline-block shadow-md mt-5 bg-black text-white'>Recipe Details</Link>
      </div>
    </div>
  )
}
